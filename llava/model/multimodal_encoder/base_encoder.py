...
class BaseVisionTower(nn.Module):
    ...
    def forward(self, images):
        if type(images) is list:
            image_features_l,image_features_m,image_features_h = [
                self._forward(image.unsqueeze(0))
                for image in images
            ]
        else:
            image_features_l,image_features_m,image_features_h = self._forward(images)

        return image_features_l,image_features_m,image_features_h
